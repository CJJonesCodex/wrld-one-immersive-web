...
export function ActiveWorldReadout({ world, progress, isMobile }: ActiveWorldReadoutProps) {
  const opacity = isMobile ? Math.min(1, Math.max(0, (progress - 0.18) / 0.08)) : Math.min(1, Math.max(0, (progress - 0.16) / 0.08));
  return (
    <div className="active-world-readout" style={{ ['--accent' as string]: world.colors.accent, opacity }}>
